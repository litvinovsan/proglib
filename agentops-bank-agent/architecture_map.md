...
flowchart TD
    ...
    class B0,B1,B2,B3,B4,B5,B6,B7 done
```
...
| 7 · финальная проверка | ✅ | полный прогон **зелёный** (50 ячеек, 856 c), тесты **13/13**, мёртвого кода 0 из 160 определений |
| сверх плана · расследование медленных обращений | ✅ | трейс выдавал ошибку тула за пустой результат (№114); пустой `required` в схеме срывал constrained decoding — 8.3 с против «не уложилось в 300 с» (№115); у вызовов модели не было таймаута вообще, поэтому один шёл два часа (№116). Итог на живом обращении: **3 часа → 275 с** |
